...
class HindsightReplayBuffer:
    ...
    def __init__(self, params):
        ...
        * **seed** (function) --- the random number seed function
        """
        self.field_names = ["state", "action", "reward", "next_state", "done", "goal"]
        self.params = params
        self.memory = deque(maxlen=self.params['buffer_size'])  
        self.batch_size = self.params['batch_size']
        self.experience = namedtuple("Experience", field_names=self.field_names)
        self.seed = random.seed(self.params['seed'].next())
        self.episodes = np.zeros((self.params['num_agents'], self.params['timesteps_per_episode'], len(self.field_names)))
    ...
    def add_to_episode(self, agent_idx, timestep, state, action, reward, next_state, done, goal):
        """Adds experience to stored episodes for HER"""
        self.episodes[agent_idx][timestep] = [state, action, reward, next_state, done, goal]

    def hindsight(self):
        """Uses hindsight into episode to set new goals and add to buffer"""
