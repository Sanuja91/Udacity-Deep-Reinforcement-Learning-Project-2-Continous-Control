...
class SimpleRolloutStorage(object):
    """Rolls out stored experiences"""
    def __init__(self, num_steps, num_processes, state_size, action_size):

        # Initalizes storage for items
        self.states = torch.zeros(num_steps, num_processes, state_size)
        ...
        self.action_log_probs = torch.zeros(num_steps, num_processes, action_size)
    ...
    def to(self, device):
        """Loads everything on to correct device"""
    ...
    def insert(self, states, actions, action_log_probs, values, rewards, masks, bad_masks):
        """Inserts storage data into storage"""
    ...
    def compute_returns(self, next_value, use_gae, gamma, gae_lambda):
        """Computes returns with or without GAE"""
        if use_gae:
    ...
    def feed_forward_generator(self, advantages, num_mini_batch = None, mini_batch_size = None):
        """Generates mini batches until a full batch has been convered"""
        ...
        for indices in sampler:
            ...
            yield states_batch, actions_batch, \
                values_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ
    ...
    def compute_returns(self,
        ...
        if use_proper_time_limits:
            if use_gae:
                ...
                gae = 0
                for step in reversed(range(self.rewards.size(0))):
                    delta = self.rewards[step] + gamma * self.values[
